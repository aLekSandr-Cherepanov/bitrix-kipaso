            ...
            <div id="elementContainer" class="column">
                ...
                <? if (!empty($arResult["DETAIL_TEXT"])): ?>
                    <div id="detailText">
                        ...
                        <? // Проверяем параметр CUSTOM_SHOW_SPECIFICATIONS и наличие свойства SPECIFICATIONS_TEXT
                        if ($arParams["CUSTOM_SHOW_SPECIFICATIONS"] == "IN_DESCRIPTION" && !empty($arResult["PROPERTIES"]["SPECIFICATIONS_TEXT"]["VALUE"]["TEXT"])): ?>
                            <div class="specifications">
                                <h3><?= Loc::getMessage("CATALOG_ELEMENT_SPECIFICATIONS_HEADING") ?: "Характеристики" ?></h3>
                                <?= $arResult["PROPERTIES"]["SPECIFICATIONS_TEXT"]["~VALUE"]["TEXT"] ?>
                            </div>
                        <? endif; ?>
                    </div>
                <? endif; ?>
                <? // Проверяем параметр вывода характеристик
                if (empty($arParams["CUSTOM_SHOW_SPECIFICATIONS"]) || $arParams["CUSTOM_SHOW_SPECIFICATIONS"] != "IN_DESCRIPTION"): ?>
                <div class="changePropertiesGroup">
                    <? // Если характеристики показываем в отдельной вкладке
                    if (!empty($arResult["PROPERTIES"]["SPECIFICATIONS_TEXT"]["VALUE"]["TEXT"]) && $arParams["CUSTOM_SHOW_SPECIFICATIONS"] == "IN_PROPERTIES_TAB"): ?>
                        <h2 class="heading"><?= Loc::getMessage("CATALOG_ELEMENT_SPECIFICATIONS_HEADING") ?: "Характеристики" ?></h2>
                        <div class="changeSpecifications"><?= $arResult["PROPERTIES"]["SPECIFICATIONS_TEXT"]["~VALUE"]["TEXT"] ?></div>
                    <? else: // Иначе используем стандартный компонент для вывода свойств
                        $APPLICATION->IncludeComponent(
                            "dresscode:catalog.properties.list",
                            "group",
                            array(

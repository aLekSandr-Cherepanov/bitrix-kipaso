...
var initFastViewApp = function () {
	...
	var changeActiveCarouselPicture = function (event) {
		...
		if (!$thisLink.hasClass("active")) {
			...
			var $appFastViewCarouselItemsLinks = $appFastView
				.find(".appFastViewPictureCarouselItemLink")
				.removeClass("active");
	...
	var carouselControlHandlerLeft = function (event) {
		...
		if (
			imageSliderCurrentPosition + 1 < $pictureCarouselItem.length - 1 ||
			repeatCarousel == true
		) {
	...
	var fastViewLoupeMove = function (event) {
		if (appFastViewPictureLoupeImageLoaded || $appFastViewPictureLoupe == false) {
			...
			if (
				($appFastViewPictureLoupe != false && mouseMathPositionX < 0) ||
				mouseMathPositionX > appFastViewPictureLoupeWidth ||
				mouseMathPositionY < 0 ||
				mouseMathPositionY > appFastViewPictureLoupeHeight
			) {
				fastViewLoupeClose();
			} else {
				if ($appFastViewPictureLoupe == false) {
					...
					var backgroundPositionX =
						(appFastViewPictureLoupeImageWidth * offsetPercentX) / 100 - 75;
					var backgroundPositionY =
						(appFastViewPictureLoupeImageHeight * offsetPercentY) / 100 - 75;
					...
					backgroundPositionX =
						backgroundPositionX > appFastViewPictureLoupeImageWidth - 150
							? appFastViewPictureLoupeImageWidth - 150
							: backgroundPositionX;
					backgroundPositionY =
						backgroundPositionY > appFastViewPictureLoupeImageHeight - 150
							? appFastViewPictureLoupeImageHeight - 150
							: backgroundPositionY;
	...
	$(document).on(
		...
		changeActiveCarouselPicture
	);
	$(document).on(
		"click",
		"#appFastView .appFastViewPictureCarouselLeftButton",
		carouselControlHandlerLeft
	);
	$(document).on(
		"click",
		"#appFastView .appFastViewPictureCarouselRightButton",
		carouselControlHandlerRight
	);
	$(document).on(
		"mousemove",

		...
		<?if(!empty($arResult["PRICE"])):?>
			<?if($arResult["CATALOG_AVAILABLE"] != "Y"):?>
				<?if($arResult["CATALOG_SUBSCRIBE"] == "Y"):?>
					<a href="#" class="addCart subscribe" data-id="<?=$arResult["ID"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/subscribe.svg" alt="" class="icon"><?=GetMessage("SUBSCRIBE_LABEL")?></a>
				<?else:?>
			...
			<?endif;?>
		<?else:?>
			<a href="#" class="addCart disabled requestPrice" data-id="<?=$arResult["ID"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/request.svg" alt="" class="icon"><?=GetMessage("TABLE_REQUEST_PRICE_BUTTON_LABEL")?></a>
		<?endif;?>
	</div>
	<div class="column">
		<div class="optional">

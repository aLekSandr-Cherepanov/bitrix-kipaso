...
(function () {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function (parameters) {
			...
			this.defaultDeliveryLogo = this.templateFolder + "/images/delivery_logo.svg";
			...
			this.locationID = parameters.locationID || parameters.locationIDByIP;
			...
			const userConsentCheckbox = this.orderSaveBlockNode.querySelector('[type="checkbox"]');
			if (userConsentCheckbox) {
				const checkboxWrap = BX.findParent(BX.findParent(userConsentCheckbox), {
					class: "checkbox"
				});
				if (userConsentCheckbox.checked) {
					BX.addClass(checkboxWrap, "checked");
				}
				...
				BX.UserConsent.onRefused = function () {
					BX.onCustomEvent(this.current, this.events.refused, []);
					BX.onCustomEvent(this, this.events.refused, [this.current]);
					this.current.inputNode.checked = false;
					this.current = null;
					this.isFormSubmitted = false;
					BX.removeClass(checkboxWrap, "checked");
				};
			}
			document.addEventListener("DOMContentLoaded", this.createFakeSelect);
			if (this.locationID) {
				document.addEventListener("DOMContentLoaded", this.changeLocation());
			}
			...
			$(".main-user-consent-request-announce-link")
				...
				.click(function () {
					setTimeout(() => {
						let consentButtons = document.querySelector(
							".main-user-consent-request-popup-buttons"
						);
						let consentButtonAcc = document.querySelector(
							".main-user-consent-request-popup-button-acc"
						);
						let consentButtonRej = document.querySelector(
							".main-user-consent-request-popup-button-rej"
						);
						let themeClass = document.getElementById("bx-soa-order").classList.value;
						if (!consentButtons) {
							consentButtons = document.querySelector(
								".main-user-consent-request-popup-buttons"
							);
							consentButtonAcc = document.querySelector(
								".main-user-consent-request-popup-button-acc"
							);
							consentButtonRej = document.querySelector(
								".main-user-consent-request-popup-button-rej"
							);
						} else {
							if (themeClass) {
								$(consentButtons).addClass(themeClass);
							}
							$(consentButtonAcc).addClass("btn-simple btn-medium");
							$(consentButtonRej).addClass("btn-simple btn-medium btn-border");
						}
					}, 10);
				});
			console.log(this);
		...
		checkMinimumPrice: function () {
			this.switchOrderSaveButtons(
				Number(this.params.MIN_SUM_TO_PAYMENT) <=
				Number(this.result.TOTAL.ORDER_TOTAL_PRICE)
			);
		},

		createFakeSelect: function () {
			const selects = BX("bx-soa-order").querySelectorAll("select:not([multiple])");

			for (let i = 0; i < selects.length; i++) {
				const optionsFake = [];
				const options = selects[i].querySelectorAll("option");
				let selectedName = options[0].text;
				const container = selects[i].parentElement;
				BX.addClass(container, "select-input-container");
				for (let j = 0; j < options.length; j++) {
					if (options[j].selected) {
						selectedName = options[j].text;
					}
					const className =
						"select-fake-option-fake" +
						(options[j].selected ? " option-fake-selected" : "");
					optionsFake.push(
						BX.create("DIV", {
							props: { className: className },
							dataset: { value: options[j].value },
							text: options[j].text,
							events: {
								click: function (event) {
									const select = container.querySelector("select");
									select.value = event.target.dataset.value;
									const fakeSelectValue =
										container.querySelector(".fake-select-value");
									fakeSelectValue.innerText = event.target.innerText;
									const list = BX.findParent(event.target, {
										className: "list-options-fake"
									});
									const selected = list.querySelector(".option-fake-selected");
									BX.removeClass(selected, "option-fake-selected");
									BX.addClass(event.target, "option-fake-selected");
				...
				const optionsFakeList = BX.create("DIV", {
					props: { className: "list-options-fake" },
					children: optionsFake
				});
				const fakeSelect = BX.create("DIV", {
					props: { className: "fake-select" },
					children: [
						BX.create("DIV", {
							props: { className: "fake-select-value" },
							text: selectedName
						}),
						optionsFakeList
					],
					events: {
						click: function (event) {
							const lists = document.querySelectorAll(".list-options-fake");
							const has = BX.hasClass(optionsFakeList, "active");
							for (let i = 0; i < lists.length; i++) {
								BX.removeClass(lists[i], "active");
							}
							if (!has) {
								BX.addClass(optionsFakeList, "active");
								BX.findParent(optionsFakeList, {
				...
				BX.append(fakeSelect, BX.findParent(selects[i], "select-input-container"));
			}
			const lists = document.querySelectorAll(".list-options-fake");
			document.addEventListener("click", function (event) {
				if (!BX.hasClass(event.target, "fake-select-value")) {
					for (let i = 0; i < lists.length; i++) {
						BX.removeClass(lists[i], "active");
					}
				}
					...
					onsuccess: BX.delegate(function (result) {
						...
						switch (eventArgs.action) {
							...
							case "showAuthForm":
								...
								this.refreshOrder(result);
								if (
									Array.isArray(result.order.ERROR) &&
									!result.order.ERROR.length
								) {
									location.reload();
								}
								break;
		...
		refreshOrder: function (result) {
			if (result.error) {
				...
				if (this.selectedDeliveryID == currentDelivery.ID) this.deliveryCachedInfo = [];
		...
		locationsCompletion: function () {
			...
			for (i in this.locations) {
				...
				if (inputStep) {
					inputSteps = inputStep.querySelectorAll(".bx-ui-slst-input-block");
					for (let i = 0; i < inputSteps.length; i++) {
						BX.addClass(inputSteps[i], "select-area");
					}
				}

				locationNode.removeAttribute("style");

				BX.addClass(inputStep, "bx-sls");
		...
		fade: function (node, nextSection) {
			...
			switch (node.id) {
				...
				case this.paySystemBlockNode.id:
					BX.remove(
						this.paySystemBlockNode.querySelector(".alert.alert-warning.alert-hide")
					);
					this.editFadePaySystemBlock();
					break;
				case this.deliveryBlockNode.id:
					BX.remove(
						this.deliveryBlockNode.querySelector(".alert.alert-warning.alert-hide")
					);
		...
		editAuthorizeForm: function (authContent) {
			...
			remember = BX.create("DIV", {
				...
				children: [
					BX.create("DIV", {
						...
						events: {
							click: function () {
								if (BX.findChild(this, { tag: "input" }, true).checked) {
									BX.addClass(this, "checked");
								} else {
									BX.removeClass(this, "checked");
								}
							}
						}
					})
			...
			authContent.appendChild(
				BX.create("DIV", { props: { className: "" }, children: [authFormNode] })
			);
		},
		...
		editSocialContent: function (authContent) {
			...
			authContent.appendChild(
				BX.create("DIV", { props: { className: "" }, children: nodes })
			);
		},
			...
			if (this.initialized.basket) {
				...
				basketContent.appendChild(
					BX.create("DIV", {
						...
						children: [
							BX.create("DIV", {
								style: { overflowX: "auto", overflowY: "hidden", minWidth: "100%" },
		...
		createBasketItemColumn: function (column, allData, active) {
			...
			return BX.create("DIV", {
				props: {
					className:
						"bx-soa-item-td bx-soa-item-properties" + (toRight ? " bx-text-right" : "")
				},
				children: [
					// BX.create('DIV', {
				...
				couponsBlock = BX.create("DIV", {
					...
					children: [
						BX.create("DIV", {
							...
							dataset: { word: this.params.MESS_COUPON_USE },
							children: [
								BX.create("INPUT", {
									props: {
										className: "form-control bx-ios-fix",
										type: "text",
										placeholder: this.params.COUPON_PLACEHOLDER
									},
									events: {
		...
		getCouponsLabel: function (active) {
			return BX.create("DIV", {
				...
				children: active
					? [
						BX.create("DIV", { props: { className: "coupon_img" } }),
		...
		addCoupon: function (coupon) {
			...
			for (var i = 0; i < couponListNodes.length; i++) {
				...
				couponListNodes[i].appendChild(
					this.getCouponNode({ text: coupon }, true, "bx-soa-coupon-item-danger")
				);
			}
		},
		...
		editRegionBlock: function (active) {
			...
			if (this.params.SHOW_QUICK_LOCATIONS !== "Y") {
				const quickLocation = this.regionBlockNode.querySelector(".quick-locations");
				BX.addClass(quickLocation, "hidden");
			} else {
				const quickLocations = this.regionBlockNode.querySelectorAll(".quick-location-tag");
				for (let i = 0; i < quickLocations.length; i++) {
					if (i >= this.params.COUNT_QUICK_LOCATIONS) {
		...
		editActiveRegionBlock: function (activeNodeMode) {
			...
			if (this.initialized.region) {
				...
				regionNodeCol = BX.create("DIV", { props: { className: "regionCol" } });
				regionNodeRow = BX.create("DIV", { props: { className: "regionRow" } });

				this.getPersonTypeControl(regionNodeRow);

				this.getProfilesControl(regionNodeRow);

				regionNodeCol.appendChild(regionNodeRow);
		...
		getPersonTypeControl: function (node) {
			...
			if (personTypesCount > 2) {
				for (i in this.result.PERSON_TYPE) {
					if (this.result.PERSON_TYPE.hasOwnProperty(i)) {
						...
						if (currentType.CHECKED == "Y") {
							var selectedName = currentType.NAME;
						}
						options.push(
							BX.create("OPTION", {
								props: {
						...
						optionsFake.push(
							BX.create("DIV", {
								props: { className: "select-fake-option-fake" },
								dataset: {
									selected: currentType.CHECKED == "Y" ? "Y" : "N",
									value: currentType.ID
								},
								text: currentType.NAME,
								events: {
									click: function (event) {
										const container = BX.findParent(event.target, {
											className: "select-input-container"
										});
										const select = container.querySelector("select");
										select.value = event.target.dataset.value;
				...
				const select = BX.create("SELECT", {
					props: { name: "PERSON_TYPE", className: "form-control select-input" },
					children: options,
					events: { change: BX.proxy(this.sendRequest, this) }
				});

				input = BX.create("DIV", {
					props: { className: "select-input-container" },
					children: [select]
				});
					...
					if (this.result.PERSON_TYPE.hasOwnProperty(i)) {
						...
						label = BX.create("LABEL", {
							...
							props: {
								className:
									"btn-person-type" +
									(currentType.CHECKED == "Y" ? " btn-person-type-checked" : "")
							}
						});
			...
			if (profilesLength) {
				if (
					...
					var select = BX.create("SELECT", {
						props: { className: "form-control select-input", name: "PROFILE_ID" },
						children: options,
						events: {
							change: BX.delegate(function () {
		...
		createPaySystemItem: function (item) {
			...
			itemNode = BX.create("DIV", {
				...
				children: [
					label,
					title,
					BX.create("DIV", { props: { className: "delivery-check" } })
				],
				events: {
		...
		editPaySystemInfo: function (paySystemNode) {
			if (
				!this.result.PAY_SYSTEM ||
				(this.result.PAY_SYSTEM.length == 0 && this.result.PAY_FROM_ACCOUNT != "Y")
			)
				return;
			...
			if (currentPaySystem) {
				...
				extPs = BX.create("DIV", { children: [label, subTitle, title, price] });
				BX.addClass(paySystemInfoContainer, "pay-checked");
				if (!currentPaySystem.DESCRIPTION) {
					BX.addClass(paySystemInfoContainer, "hidden");
				}
			}
			...
			if (innerPs) paySystemNode.appendChild(innerPs);
			paySystemInfoContainer.appendChild(
				BX.create("DIV", { props: { className: "bx-soa-pp-info" } })
			);
		},
		...
		getInnerPaySystem: function () {
			...
			var accountOnly =
				...
				isSelected =
					this.result.PAY_CURRENT_ACCOUNT && this.result.PAY_CURRENT_ACCOUNT == "Y",
			...
			return BX.create("LABEL", {
				...
				children: [
					hiddenInput,
					label,
					BX.create("DIV", { children: [subTitle, title, innerPsDesc] })
				]
			});
		},
		...
		editFadePaySystemContent: function (node) {
			...
			if (selectedPaySystem && selectedPaySystem.NAME) {
				...
				addedHtml += '<img src="' + imgSrc + '" style="height:18px;" alt="">';
				addedHtml +=
					"<strong>" + BX.util.htmlspecialchars(selectedPaySystem.NAME) + "</strong>";
				addedHtml += "</div>";
			}
		...
		selectPaySystem: function (event) {
			...
			var target = event.target || event.srcElement,
				innerPaySystemSection =
					this.paySystemBlockNode.querySelector("div.bx-soa-pp-inner-ps"),
		...
		editDeliveryInfo: function (deliveryNode) {
			...
			// Егор доставка самовывоз оригинальный код
			...
			if (currentDelivery.PRICE >= 0) {
				let priceText;
				if (currentDelivery.ID == '76') {
					priceText = this.params.MESS_PRICE_FREE_INDIVIDUAL;
				} else if (parseFloat(currentDelivery.PRICE) === 0) {
					this.params.MESS_PRICE_FREE_PICKUP;
				} else {
					priceText = this.getDeliveryPriceNodes(currentDelivery);
				}
			...
			deliveryInfoContainer.appendChild(
				BX.create("DIV", {
					props: { className: "bx-soa-pp-company" },
					children: [label, subTitle, title, infoList, clear, extraServicesNode]
				})
			);
		...
		getDeliveryPriceNodes: function (delivery) {
			...
			if (
				...
				priceNodesArray = delivery.PRICE
		...
		getDeliveryExtraServices: function (delivery) {
			...
			for (i in delivery.EXTRA_SERVICES) {
				...
				if (currentService.editControl.indexOf("this.checked") == -1) {
					...
					temp.innerHTML = currentService.editControl;
					if (!!temp.querySelector("select")) {
						currentService.editControl =
							'<div class="input-container">' + currentService.editControl + "</div>";
					}
					...
					serviceNode = BX.create("DIV", {
						props: {
							className: "checkbox" + (currentService.value === "Y" ? " checked" : "")
						},
						children: [
							BX.create("LABEL", {
								html:
		...
		createDeliveryItem: function (item) {
			...
			if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== "undefined") {
				...
				if (item.ID == '76') {
					deliveryCostText = this.params.MESS_PRICE_FREE_INDIVIDUAL;
				} else if (item.PRICE == 0) {
					deliveryCostText = this.params.MESS_PRICE_FREE_PICKUP;
				} else if (typeof item.DELIVERY_DISCOUNT_PRICE !== "undefined") {
					deliveryCostText = item.DELIVERY_DISCOUNT_PRICE_FORMATED;
				} else {
					deliveryCostText = item.PRICE_FORMATED;
				}

				textNodes.push(
					BX.create("DIV", {
						props: { className: "bx-soa-pp-delivery-cost" },
						html: deliveryCostText
					})
				);
		...
		textNodes.push(
			BX.create("DIV", {
				props: {
					className: "bx-soa-pp-delivery-cost bx-soa-pp-delivery-calculate"
				},
				html: this.params.CALCULATE
			})
		);
	}
...
editActivePickUpBlock: function (activeNodeMode) {
	...
	if (this.initialized.pickup) {
		...
		pickUpContentCol = BX.create("DIV", { props: { className: "pickup-container" } });
...
checkPickUpShow: function () {
	...
	if (stores) {
		console.log(this.params.SHOW_MAP_FOR_DELIVERIES);
	}
...
resizeMapContainers: function () {
	...
	if (this.params.SHOW_PICKUP_MAP === "Y" && pickUpMapContainer) {
		var resizeBy = BX.findParent(pickUpMapContainer);
		if (
			document.documentElement.clientWidth < 1366 &&
			document.documentElement.clientWidth >= 400
		) {
			pickUpMapContainer.style.height = "280px";
		} else if (document.documentElement.clientWidth < 400) {
			pickUpMapContainer.style.height = "240px";
		} else {
			pickUpMapContainer.style.height = "345px";
		}
		pickUpMapContainer.style.width = "0px";
		pickUpMapContainer.style.width = resizeBy.clientWidth + "px";
	}

	if (this.params.SHOW_MAP_IN_PROPS === "Y" && propertyMapContainer) {
		var resizeBy = BX.findParent(propertyMapContainer);
		if (document.documentElement.clientWidth < 1366) {
			propertyMapContainer.style.height = "280px";
		} else if (document.documentElement.clientWidth < 400) {
			propertyMapContainer.style.height = "240px";
		} else {
			propertyMapContainer.style.height = "345px";
		}
		propertyMapContainer.style.width = "0px";
		propertyMapContainer.style.width = resizeBy.clientWidth + "px";
	}
},
...
editPickUpList: function (isNew) {
	...
	if (!!isNew) {
		container = this.pickUpHiddenBlockNode.querySelector(
			".bx_soa_pickup>.pickup-container"
		);
		if (!container)
...
getStoreInfoHtml: function (currentStore) {
	...
	if (currentStore.DESCRIPTION)
		html +=
					/*BX.message('SOA_PICKUP_DESC') + ': ' + */ BX.util.htmlspecialchars(
			currentStore.DESCRIPTION
		) + "<br>";

	if (currentStore.SCHEDULE)
		html +=
					/*BX.message('SOA_PICKUP_WORK') + ': ' + */ BX.util.htmlspecialchars(
			currentStore.SCHEDULE
		) + "<br>";

	if (currentStore.PHONE)
		html +=
					/*BX.message('SOA_PICKUP_PHONE') + ': ' + */ BX.util.htmlspecialchars(
			currentStore.PHONE
		) + "<br>";
				...
				children: [
					BX.create("A", {
						props: {
							href: "",
							className:
								"btn btn-sm" + (options.selected ? " btn-default" : "")
						},
...
editPickUpRecommendList: function (geoLocation) {
	...
	if (!container) {
		container = this.pickUpBlockNode.querySelector(".bx_soa_pickup>.pickup-container");
	}
...
selectStore: function (event) {
	...
	if (storeItem && storeInput) {
		...
		const buttons = BX.findParent(storeItem).querySelectorAll(
			".bx-soa-pickup-l-item-btn .btn"
		);
		for (let i = 0; i < buttons.length; i++) {
			buttons[i].innerText = this.params.MESS_SELECT_PICKUP;
			BX.removeClass(buttons[i], "btn-default");
		}

		if (BX.hasClass(target, "btn")) {
			target.innerText = this.params.MESS_SELECTED_PICKUP;
			BX.addClass(target, "btn-default");
		} else {
			const button = BX.findParent(target).querySelector(".btn");
			button.innerText = this.params.MESS_SELECTED_PICKUP;
			BX.addClass(button, "btn-default");
		}
	}
},
...
getPropertyRowNode: function (property, propsItemsContainer, disabled) {
	...
	if (disabled) {
		...
		textHtml += BX.util.htmlspecialchars(property.getName()).trim();
			...
			if (property.isMultiple()) {
				propContainer.appendChild(
					BX.create("DIV", {
						...
						props: { className: "btn btn-sm btn-default btn-border" },
	...
	if (disabled) {
		...
		if (prop) {
			...
			for (i = 0; i < fileLinks.length; i++) {
				...
				BX.addClass(fileLinks[i], "file-link");
		...
		if (saved) {
			...
			fileLinks = saved.querySelectorAll("a");
		...
		if (propContainer) {
			...
			propContainer = BX.create("DIV", {
				props: { className: "soa-property-container soa-property-files" }
			});
			property.appendTo(propContainer);
			propsItemNode.appendChild(propContainer);
			this.alterProperty(property.getSettings(), propContainer);

			fileLinks = propContainer.querySelectorAll("a");
...
insertEnumProperty: function (property, propsItemNode, disabled) {
	...
	const select = propContainer.querySelector("select");
	if (select) {
		if (select.hasAttribute("multiple")) {
			BX.addClass(propContainer, "select-multiple-area");
			const options = select.querySelectorAll("option");
			const optionsFake = [];
			for (let i = 0; i < options.length; i++) {
				optionsFake.push(
					BX.create("DIV", {
						props: { className: "select-multiple-fake-option" },
						html: options[i].innerHTML,
						events: {
							click: function (event) {
								if (!event.ctrlKey && event.pointerType !== "touch") {
									const fakeOptions = document.querySelectorAll(
										".select-multiple-fake-option"
									);
									for (let j = 0; j < options.length; j++) {
										options[j].selected = false;
										BX.removeClass(fakeOptions[j], "selected-option");
									}
								}
								BX.toggleClass(this, "selected-option");
								if (BX.hasClass(this, "selected-option")) {
			...
			propContainer.appendChild(
				BX.create("DIV", {
					props: { className: "select-multiple-fake" },
					children: optionsFake
				})
			);
		} else {
			BX.addClass(propContainer, "select-area");
		}
	}

	const checkboxes = propContainer.querySelectorAll("[type=checkbox]");
	if (checkboxes.length) {
		for (i = 0; i < checkboxes.length; i++) {
			const parent = BX.findParent(BX.findParent(checkboxes[i]));
			BX.addClass(parent, "checkbox");
	...
	const radios = propContainer.querySelectorAll("[type=radio]");
	if (radios.length) {
		for (i = 0; i < radios.length; i++) {
			const parent = BX.findParent(BX.findParent(radios[i]));
			BX.addClass(parent, "radio");
			parent.onclick = function () {
				if (parent.querySelector('[type="radio"]').checked) {
					const buttons = BX.findParent(parent).querySelectorAll("[type=radio]");
					for (i = 0; i < buttons.length; i++) {
						BX.removeClass(BX.findParent(BX.findParent(buttons[i])), "checked");
					}
					BX.addClass(parent, "checked");
				} else {
					BX.removeClass(parent, "checked");
...
insertYNProperty: function (property, propsItemNode, disabled) {
	...
	if (disabled) {
		...
		propContainer = BX.create("LABEL", {
			props: { className: "soa-property-container checkbox_property" }
		});
		property.appendTo(propContainer);
		propsItemNode.appendChild(propContainer);
		this.alterProperty(property.getSettings(), propContainer);
		this.bindValidation(property.getId(), propContainer);
		BX.addClass(propsItemNode, "YNContainer");
		const input = propsItemNode.querySelector("input[type=checkbox]");
		input.setAttribute("id", "soa-property-" + property.getId());
		propsItemNode.onclick = function () {
			if (propsItemNode.querySelector('[type="checkbox"]').checked) {
				BX.addClass(propContainer, "checked");
			} else {
				BX.removeClass(propContainer, "checked");
			}
		};
	}
},
...
alterProperty: function (settings, propContainer) {
	...
	if (textNode) {
		...
		if (settings.IS_PHONE == "Y") {
			textNode.setAttribute("autocomplete", "off");
			textNode.setAttribute("type", "text");
			textNode.setAttribute("inputmode", "tel");
			if (this.params.USE_MASKED === "Y") {
				$(textNode).masked({ mask: this.params.MASKED_FORMAT });
			}
		}
	...
	inputs = propContainer.querySelectorAll("input[type=email]");
	for (i = 0; i < inputs.length; i++) {
		inputs[i].placeholder = settings.DESCRIPTION;
		BX.addClass(inputs[i], "form-control bx-soa-customer-input bx-ios-fix");
	}
	...
	for (i = 0; i < inputs.length; i++) {
		...
		BX.addClass(BX.findParent(inputs[i]), "selectArea");
	}
...
alterDateProperty: function (settings, inputText) {
	...
	const addonInput = parentNode.querySelector("input[type=text]");

	addonInput.addEventListener("click", function () {
		addon.click();
	});

	BX.addClass(parentNode.querySelector("input[type=text]"), "input-with-addon");
...
editPropsComment: function (propsNode) {
	...
	propsCommentContainer = BX.create("DIV", { props: { className: "" } });
	label = BX.create("LABEL", {
...
editTotalBlock: function () {
	...
	if (Number(total.DISCOUNT_PRICE)) {
		this.totalInfoBlockNode.appendChild(
			BX.create("SPAN", {
				props: { className: "discount_procent" },
				html: `-${Math.round(Number(total.DISCOUNT_PRICE) / (Number(total.PRICE_WITHOUT_DISCOUNT_VALUE) / 100))}%`
			})
		);
	}
	...
	if (this.options.showPayedFromInnerBudget) {
		...
		if (
			...
			this.totalInfoBlockNode.appendChild(this.createMinimumSumBlock());
		}
		this.totalInfoBlockNode.appendChild(
			this.createTotalUnit(
	...
	if (this.options.showPriceWithoutDiscount) {
		this.totalInfoBlockNode.appendChild(
			this.createTotalUnit(
				BX.message("SOA_SUM_SUMMARY_WITHOUT_DISCOUNT"),
	...
	// Егор менял доставку на "Самовывоз"
	// params = {};
	// curDelivery = this.getSelectedDelivery();
	// deliveryError =
	// 	curDelivery && curDelivery.CALCULATE_ERRORS && curDelivery.CALCULATE_ERRORS.length;
	...
	// 	) {
	// 		deliveryValue +=
	// 			'<br><span class="bx-price-old">' + curDelivery.PRICE_FORMATED + "</span>";
	// 	}
	...
	if (deliveryError) {
		...
		if (curDelivery && curDelivery.ID == '76') {
			// Для "Желаемый адрес доставки" — пишем "Индивидуальный расчёт"
			deliveryValue = this.params.MESS_PRICE_FREE_INDIVIDUAL;
			params.free = true;
		} else if (curDelivery && parseFloat(total.DELIVERY_PRICE) === 0) {
			// Для всех самовывозов — пишем "Самовывоз"
			deliveryValue = this.params.MESS_PRICE_FREE_PICKUP;
			params.free = true;
		} else {
			deliveryValue = total.DELIVERY_PRICE_FORMATED;
		}
	...
	if (!this.result.SHOW_AUTH && this.params.SHOW_TOTAL_ORDER_BUTTON === "Y") {
		this.totalInfoBlockNode.appendChild(
			BX.create("DIV", {
				props: {
					className:
	...
	if (this.params.SHOW_COUPONS === "Y" && this.params.SHOW_COUPONS_TOTAL === "Y") {
...
createTotalUnit: function (name, value, params) {
	...
	if (params.delivery) {
		className += " bx-soa-cart-total-line-delivery";
	}
	...
	return BX.create("DIV", {
		...
		children: [
			...
			BX.create("SPAN", { props: { className: "bx-soa-cart-border" } }),
			BX.create("SPAN", {
				props: {
					className:
						"bx-soa-cart-d" +
...
createMinimumSumBlock: function () {
	const priceText = BX.Currency.currencyFormat(
		this.params.MIN_SUM_TO_PAYMENT,
		this.currency,
		true
	);
	return BX.create("DIV", {
		props: { className: "minimum-sum-block" },
		children: [
			BX.create("DIV", {
				props: { className: "minimum-sum-block_top" },
				children: [
					BX.create("DIV", {
						props: { className: "minimum-sum-block_left" },
						children: [
							BX.create("DIV", {
								props: { className: "minimum-sum-block_name" },
								text: this.params.MIN_SUM_TO_PAYMENT_TITLE
							}),
							BX.create("DIV", {
								props: { className: "minimum-sum-block_sum" },
								html: priceText
							})
						]
					}),
					BX.create("DIV", {
						props: { className: "bx-soa-pp-info" }
					})
				]
			}),
			BX.create("A", {
				props: {
					className: "minimum-sum-block_link btn btn-default btn-lg",
					href: this.params.EMPTY_BASKET_HINT_PATH
				},
				text: this.params.ADDED_TO_ORDER
			})
		]
	});
},
...
changeLocation: function () {
	const props = this.result.ORDER_PROP.properties;
	for (let i in props) {
		if (
			props[i].TYPE === "LOCATION" &&
			$(`input[name='ORDER_PROP_${props[i].ID}']`).val() != this.locationID
		) {
			$(`input[name='ORDER_PROP_${props[i].ID}']`).val(this.locationID);
			this.sendRequest();
		}

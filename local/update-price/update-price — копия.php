...
// проверяем что папка есть если нет создаем, также файл лога
$logDir = __DIR__ . '/log';
...
$lockFile = $logDir . '/update-price.lock';
$lockFp = @fopen($lockFile, 'c+');
if ($lockFp === false) {
    $log('ERROR: cannot open lock file ' . $lockFile);
    exit("Не удалось открыть lock-файл: {$lockFile}\n");
}

if (!@flock($lockFp, LOCK_EX | LOCK_NB)) {
    $log('SKIP: already running');
    exit("Скрипт уже запущен (lock)\n");
}
